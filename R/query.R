...
query <- function(query, lat, long, radius, place, since, until, from, to, replies, minLikes,
  ...
  place_  = if (is.na(place_)) '' else { paste0("near:", place_) }
  ...
  q       = paste(sTerm, from_, to_, until_, since_, place_, geo, minLikes_, minReplies_, minRetweets_, replies_,  verified_, hasImage_, hasVideo_, hasMedia_, hasLinks_, url_)
